...
const HEIGHTS = { sm: 'h-[195px]', md: 'h-[262px]', lg: 'h-[330px]' };
    ...
    >
      ...
      <div class="relative w-full aspect-[2/3] bg-[var(--surface)] overflow-hidden rounded-lg">
        <Show when={props.item.poster_path} fallback={
          <div class="w-full h-full flex items-center justify-center text-sm font-semibold" style={{ color: 'var(--text)' }}>
            No poster
          </div>
        }>
          <img
            src={imageUrl(props.item.poster_path, size() === 'lg' ? 'w500' : 'w342')}
