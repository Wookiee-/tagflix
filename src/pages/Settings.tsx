...
export default function SettingsPage() {
  ...
  return (
    <div class="p-6 md:p-10 pb-24 animate-fade-in max-w-3xl mx-auto">
      ...
      {/* ═══ Theme Skin ═══ */}
      <div class="mb-14">
        <h3 class="text-xs font-bold uppercase tracking-widest mb-5" style={{ color: 'var(--accent)' }}>Theme Skin</h3>
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    'box-shadow': selected() ? '0 0 16px var(--accent-glow)' : 'none',
                  }}
                  onClick={() => { setSkinState(s.id); applySkin(s.id); }}
                >
                  <div class="flex gap-1.5 mb-3">
                    <div class="w-4 h-4 rounded-full" style={{ background: s.bg, border: '1px solid rgba(255,255,255,0.1)' }} />
                    <div class="w-4 h-4 rounded-full" style={{ background: s.surface, border: '1px solid rgba(255,255,255,0.1)' }} />
                    <div class="w-4 h-4 rounded-full" style={{ background: selected() ? 'var(--accent)' : 'rgba(255,255,255,0.08)' }} />
                  </div>
                  <p class="text-sm font-bold" style={{ color: selected() ? 'white' : 'var(--text)' }}>
                    {s.name}
                  </p>
                  <p class="text-[11px] text-white/30 mt-0.5">{s.desc}</p>
                  <Show when={selected()}>
                    <div class="absolute top-3 right-3" style={{ color: 'var(--accent)' }}>
      ...
      {/* ═══ Accent Color ═══ */}
      <div class="mb-14">
        <h3 class="text-xs font-bold uppercase tracking-widest mb-5" style={{ color: 'var(--accent)' }}>Accent Color</h3>
      ...
      {/* ═══ Streaming Source ═══ */}
      <div class="mb-14">
        <h3 class="text-xs font-bold uppercase tracking-widest mb-5" style={{ color: 'var(--accent)' }}>Streaming Source</h3>
        <div class="flex flex-col gap-2">
          <For each={SOURCES}>
            {(s) => {
              ...
              return (
                <button
                  class="flex items-center gap-3 p-4 rounded-xl transition-all duration-200 text-left"
                  style={{
                    background: selected() ? 'rgba(255,255,255,0.06)' : 'rgba(255,255,255,0.02)',
                    border: selected() ? '2px solid var(--accent)' : '2px solid rgba(255,255,255,0.06)',
                  }}
                ...
                >
                  <div
                    class="w-4 h-4 rounded-full shrink-0 flex items-center justify-center transition-all duration-200"
                    style={{
                      border: selected() ? '2px solid var(--accent)' : '2px solid rgba(255,255,255,0.2)',
                      background: selected() ? 'var(--accent)' : 'transparent',
                      'box-shadow': selected() ? '0 0 10px var(--accent-glow)' : 'none',
                    }}
                  >
                    <Show when={selected()}>
      ...
      {/* ═══ Player ═══ */}
      <div class="mb-14">
        <h3 class="text-xs font-bold uppercase tracking-widest mb-5" style={{ color: 'var(--accent)' }}>Player</h3>
      ...
      {/* ═══ Data ═══ */}
      <div class="mb-14">
        <h3 class="text-xs font-bold uppercase tracking-widest mb-5 text-red-400">Data</h3>
        <div class="flex items-center justify-between p-4 rounded-xl" style={{ background: 'rgba(255,255,255,0.02)', border: '2px solid rgba(255,255,255,0.06)' }}>
          <div>
